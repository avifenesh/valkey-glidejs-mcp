...
import { IOREDIS_DATASET, NODE_REDIS_DATASET, GLIDE_SURFACE } from "../data/api/mappings.js";
...
export function registerApiTools(mcp: McpServer) {
  ...
  mcp.tool(
    ...
    async (args) => {
      ...
      const diff = results.map((r) => ({ symbol: r.symbol, paramsDiff: r.paramsDiff, returnDiff: r.returnDiff, quirks: r.quirks }));
      return {
  ...
  mcp.tool(
    ...
    async () => {
      ...
      return { structuredContent: { categories: list }, content: [{ type: "text", text: JSON.stringify(list) }] } as any;
    }
  );
