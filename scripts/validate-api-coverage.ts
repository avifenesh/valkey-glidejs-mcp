...
import { GLIDE_SURFACE, IOREDIS_DATASET, NODE_REDIS_DATASET, getAllGlideMethods as getGlideMethods } from '../src/data/api/mappings.js';
import { COMPREHENSIVE_IOREDIS_MAPPINGS, COMPREHENSIVE_NODE_REDIS_MAPPINGS, COMPREHENSIVE_GLIDE_MAPPINGS } from '../src/data/api/comprehensive-mappings.js';
...
function loadGlideInventory(): ApiInventory {
  const inventoryPath = '/Users/avifen/valkey-glidejs-mcp/valkey-glidejs-mcp/glide-api-inventory.json';
...
function categorizeMethod(methodName: string): string {
  const categories: Record<string, string[]> = {
    'strings': ['get', 'set', 'mget', 'mset', 'del', 'exists', 'incr', 'decr', 'append', 'strlen', 'setex', 'psetex', 'setnx', 'getset', 'getrange', 'setrange'],
    'hashes': ['hget', 'hset', 'hmget', 'hmset', 'hdel', 'hexists', 'hkeys', 'hvals', 'hlen', 'hincrby', 'hgetall', 'hsetnx', 'hincrbyfloat', 'hscan', 'hstrlen', 'hrandfield'],
    'lists': ['lpush', 'rpush', 'lpop', 'rpop', 'llen', 'lrange', 'lindex', 'lset', 'ltrim', 'lrem', 'linsert', 'rpoplpush', 'brpoplpush', 'blpop', 'brpop', 'lmove', 'blmove'],
    'sets': ['sadd', 'srem', 'smembers', 'sismember', 'scard', 'sinter', 'sunion', 'sdiff', 'sinterstore', 'sunionstore', 'sdiffstore', 'spop', 'srandmember', 'smove', 'sscan', 'smismember'],
    'sortedsets': ['zadd', 'zrem', 'zrange', 'zrank', 'zscore', 'zcount', 'zincrby', 'zcard', 'zrevrange', 'zrevrank', 'zunion', 'zinter', 'zdiff', 'zrangebyscore', 'zremrangebyscore', 'zremrangebyrank', 'zscan', 'zpopmin', 'zpopmax', 'bzpopmin', 'bzpopmax'],
    'geo': ['geoadd', 'geodist', 'geohash', 'geopos', 'georadius', 'geosearch', 'geosearchstore'],
    'streams': ['xadd', 'xread', 'xlen', 'xrange', 'xdel', 'xgroup', 'xreadgroup', 'xack', 'xpending', 'xclaim', 'xinfo', 'xtrim', 'xrevrange'],
    'pubsub': ['publish', 'subscribe', 'psubscribe', 'unsubscribe', 'punsubscribe', 'pubsub'],
    'transactions': ['multi', 'exec', 'discard', 'watch', 'unwatch'],
    'scripting': ['eval', 'evalsha', 'script', 'fcall', 'fcall_ro', 'function'],
    'connection': ['ping', 'echo', 'select', 'auth', 'quit', 'hello', 'client', 'config'],
    'server': ['info', 'config', 'flushdb', 'flushall', 'dbsize', 'time', 'lastsave', 'save', 'bgsave', 'shutdown', 'slaveof', 'replicaof', 'role', 'command'],
    'bitmap': ['setbit', 'getbit', 'bitcount', 'bitpos', 'bitop', 'bitfield'],
    'hyperloglog': ['pfadd', 'pfcount', 'pfmerge'],
    'json': ['json'],
    'cluster': ['cluster', 'readonly', 'readwrite', 'asking'],
    'acl': ['acl'],
    'generic': ['expire', 'expireat', 'ttl', 'pttl', 'persist', 'rename', 'renamenx', 'type', 'dump', 'restore', 'touch', 'scan', 'randomkey', 'copy', 'migrate', 'wait', 'object']
  };
...
async function validateCoverage(): Promise<ValidationResult> {
  ...
  const migrationGaps = {
    ...
    nodeRedis: Array.from(missingApis).filter(api => !nodeRedisMapped.has(api))
  };
  ...
  console.log('\n📊 MIGRATION COVERAGE:');
  console.log('─'.repeat(60));
  console.log(`ioredis gaps:     ${migrationGaps.ioredis.length} APIs without migration path`);
  console.log(`node-redis gaps:  ${migrationGaps.nodeRedis.length} APIs without migration path`);
  
  // Save detailed report
  const reportPath = '/Users/avifen/valkey-glidejs-mcp/valkey-glidejs-mcp/api-coverage-report.json';
...
async function validateBehaviorDifferences() {
  ...
  const behaviorPath = '/Users/avifen/valkey-glidejs-mcp/valkey-glidejs-mcp/behavior-differences.json';
