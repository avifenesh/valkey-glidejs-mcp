...
test("Comprehensive MCP Server Validation", async (t) => {
  ...
  const registeredTools = Object.entries((mcp as any)._registeredTools || {}).map(
  ...
  await t.test("Tools with no parameters work correctly", async () => {
    ...
    const clientBasicTool = registeredTools.find(t => t.name === "gen.clientBasic");
    ...
    // Test api.categories
    const categoriesTool = registeredTools.find(t => t.name === "api.categories");
  ...
  await t.test("Tools with required parameters validate input", async () => {
    ...
    const findEquivTool = registeredTools.find(t => t.name === "api.findEquivalent");
...
`;
    ...
    assert.ok(migratedCode.includes("@valkey/valkey-glide"));
    assert.ok(migratedCode.includes("GlideClient"));
    assert.ok(!migratedCode.includes("createClient()"), "Should not have createClient() pattern");
  });
...
`;
    ...
    assert.ok(result.structuredContent, "verify.static should return structured content");
    assert.ok(result.structuredContent.errors.length > 0, "Should detect errors");
    assert.ok(result.structuredContent.issues.length > 0, "Should detect issues");
  });
  ...
  await t.test("Generator tools produce valid GLIDE code", async () => {
    ...
    for (const toolName of generatorTools) {
      ...
      assert.ok(code.includes("@valkey/valkey-glide"), `${toolName} should import GLIDE`);
      assert.ok(code.includes("GlideClient") || code.includes("GlideClusterClient"), 
        `${toolName} should use GLIDE client`);
      assert.ok(!code.includes("new Redis"), `${toolName} should not use ioredis constructor`);
      assert.ok(!code.includes("createClient()"), `${toolName} should not use node-redis pattern`);
    }
  });

  await t.test("Data enrichment tool works", async () => {
    const dataEnrichTool = registeredTools.find(t => t.name === "data.enrich");
  ...
  await t.test("Docs tools are functional", async () => {
    const listSourcesTool = registeredTools.find(t => t.name === "docs.listSources");
  ...
  await t.test("Tool schemas are properly defined", () => {
    ...
    const toolsWithParams = [
      ...
      { name: "gen.rateLimiter", expectedParams: ["key", "points", "duration"] },
    ...
    for (const { name, expectedParams } of toolsWithParams) {
      ...
      if (schema && schema.shape) {
        ...
        for (const param of expectedParams) {
          assert.ok(schemaKeys.includes(param), 
