...
export const COMPREHENSIVE_IOREDIS_MAPPINGS: ApiDataset = {
  ...
  entries: [
    ...
    },
    {
      ...
      description:
        "Execute geoadd command - NOTE: GLIDE expects a Map, not array/object",
      examples: {
        source:
          "await redis.geoadd('key', { long: 13.361, lat: 38.115, member: 'Palermo' })",
        glide:
          "await client.geoadd('key', new Map([['Palermo', { longitude: 13.361, latitude: 38.115 }]]))",
      },
    },
    {
      category: "geo",
    ...
    },
    {
      ...
      description:
        "Execute scan command - NOTE: GLIDE returns [cursor, keys[]] array",
      examples: {
        source:
          "const result = await redis.scan(cursor); // returns {cursor, keys}",
        glide:
          "const [newCursor, keys] = await client.scan(cursor); // returns [cursor, keys[]]",
      },
    },
    {
      category: "generic",
