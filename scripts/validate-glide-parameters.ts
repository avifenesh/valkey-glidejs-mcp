...
interface MethodMeta { minArity: number; maxArity: number | null; paramTypes?: string[]; }
interface InventoryIndex { [name: string]: MethodMeta[]; }
...
function loadInventory(file: string): InventoryIndex {
  ...
  for (const k of Object.keys(raw)) {
    ...
    if (Array.isArray(arr)) {
      arr.forEach((m: any) => {
        if (!m.name) return;
        (index[m.name] ||= []).push({ minArity: m.minArity ?? 0, maxArity: m.maxArity ?? (m.maxArity === 0 ? 0 : null), paramTypes: m.paramTypes });
      });
    }
  }
  return index;
...
interface CallRecord { file: string; method: string; argCount: number; valid: boolean; reason?: string; detail?: string; dynamic?: boolean; }

function acceptable(meta: MethodMeta[], argCount: number): boolean {
  return meta.some(m => argCount >= m.minArity && (m.maxArity === null || argCount <= m.maxArity));
}

function shapeMismatch(meta: MethodMeta[], nodeArgs: ts.NodeArray<ts.Expression>): string | null {
  ...
  for (const variant of meta) {
    ...
    const expectsObject = /(\{|Record<|Options|Configuration|Config)/.test(firstType);
    if (expectsObject) {
...
function scanTsFiles(root: string, inventory: InventoryIndex): CallRecord[] {
  ...
  function walk(dir: string) {
    for (const e of fs.readdirSync(dir)) {
      ...
      if (stat.isDirectory()) walk(full); else if (e.endsWith('.ts')) {
        ...
        const visit = (node: ts.Node) => {
          if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
            ...
            // Heuristic filters
            const arrayMethodBlacklist = new Set(['forEach','map','filter','reduce','some','every','push','pop','includes','slice']);
            ...
            if (!meta) {
              records.push({ file: path.relative(root, full), method, argCount, valid: false, reason: 'unknown-method' });
            } else if (!acceptable(meta, argCount)) {
              records.push({ file: path.relative(root, full), method, argCount, valid: false, reason: 'arity-mismatch' });
            } else {
              const shape = shapeMismatch(meta, node.arguments);
              if (shape) {
                records.push({ file: path.relative(root, full), method, argCount, valid: false, reason: 'shape-mismatch', detail: shape });
              } else {
                records.push({ file: path.relative(root, full), method, argCount, valid: true });
              }
            }
          }
          ...
          if (ts.isCallExpression(node) && ts.isElementAccessExpression(node.expression)) {
            ...
            if (!/^([a-z]*)(glide)?client$/i.test(finalToken)) {
              ...
              if (!/(json|ft)client$/i.test(finalToken)) return ts.forEachChild(node, visit);
            }
            const argumentExpr = node.expression.argumentExpression;
            let methodName: string | null = null;
            let dynamic = false;
            if (argumentExpr) {
              if (ts.isStringLiteral(argumentExpr)) methodName = argumentExpr.text; else dynamic = true;
            }
            const argCount = node.arguments.length;
            if (methodName) {
              const meta = inventory[methodName];
              if (!meta) {
                records.push({ file: path.relative(root, full), method: methodName, argCount, valid: false, reason: 'unknown-method', dynamic });
              } else if (!acceptable(meta, argCount)) {
                records.push({ file: path.relative(root, full), method: methodName, argCount, valid: false, reason: 'arity-mismatch', dynamic });
              } else {
                const shape = shapeMismatch(meta, node.arguments);
                if (shape) {
                  records.push({ file: path.relative(root, full), method: methodName, argCount, valid: false, reason: 'shape-mismatch', detail: shape, dynamic });
                } else {
                  records.push({ file: path.relative(root, full), method: methodName, argCount, valid: true, dynamic });
                }
              }
            } else {
              // dynamic unknown name
              records.push({ file: path.relative(root, full), method: '<dynamic>', argCount, valid: true, reason: undefined, dynamic: true });
            }
          }
...
function scanMarkdown(root: string, inventory: InventoryIndex): CallRecord[] {
  ...
  function processFile(mdPath: string) {
    ...
    while ((match = fenceRegex.exec(text))) {
      const code = match[2];
      const sf = ts.createSourceFile(mdPath, code, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
      const visit = (node: ts.Node) => {
        if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
          ...
          const meta = inventory[method];
          if (!meta) {
            records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: false, reason: 'unknown-method' });
          } else if (!acceptable(meta, argCount)) {
            records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: false, reason: 'arity-mismatch' });
          } else {
            const shape = shapeMismatch(meta, node.arguments);
            if (shape) {
              records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: false, reason: 'shape-mismatch', detail: shape });
            } else {
              records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: true });
            }
          }
        }
        ts.forEachChild(node, visit);
  ...
  const inlinePattern = /(\b[a-zA-Z0-9_]*(?:glide)?client|\bjsonclient|\bftclient)\.([a-zA-Z0-9_]+)\s*\(([^)]*)\)/gi;
  ...
  const finalToken = obj.split(/\.|!|\?/).pop() || '';
  if (!/^([a-z]*)(glide)?client$/i.test(finalToken) && !/(json|ft)client$/i.test(finalToken)) continue;
      ...
      if (!meta) {
        ...
        if (meta.some(m => m.paramTypes && /(\{|Record<|Options|Configuration|Config)/.test(m.paramTypes[0] || ''))) {
          if (/^\s*['"][^'"]*['"]\s*$/.test(argsRaw.split(',')[0] || '')) {
            records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: false, reason: 'shape-mismatch', detail: 'expected object-like first argument' });
            continue;
          }
        }
        records.push({ file: path.relative(process.cwd(), mdPath), method, argCount, valid: true });
  ...
  function walkDocs(dir: string) {
    ...
    for (const e of fs.readdirSync(dir)) {
      const full = path.join(dir, e);
      const stat = fs.statSync(full);
      if (stat.isDirectory()) walkDocs(full); else if (e.endsWith('.md')) processFile(full);
    }
  }
  ['README.md', 'docs'].forEach(p => {
    ...
    if (fs.existsSync(full)) {
      ...
      if (st.isDirectory()) walkDocs(full); else processFile(full);
    }
  });
  return records;
}
...
function main() {
  ...
  fs.writeFileSync('glide-parameter-validation.json', JSON.stringify(report, null, 2));
  console.log('Parameter validation complete. Output: glide-parameter-validation.json');
  if (summary.invalid > 0) {
    console.error(`Found ${summary.invalid} invalid method usages (unknown or arity mismatch).`);
    process.exitCode = 1;
  } else {
    console.log('All detected usages and examples conform to inventory method arity.');
  }
}
