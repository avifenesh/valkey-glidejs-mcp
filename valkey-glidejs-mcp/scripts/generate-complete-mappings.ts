...
function getCategory(methodName: string): string {
  const categories: Record<string, string[]> = {
    'strings': ['get', 'set', 'mget', 'mset', 'del', 'exists', 'incr', 'decr', 'append', 'strlen', 'setex', 'psetex', 'setnx', 'getset', 'getrange', 'setrange', 'getdel', 'getex'],
    'hashes': ['hget', 'hset', 'hmget', 'hmset', 'hdel', 'hexists', 'hkeys', 'hvals', 'hlen', 'hincrby', 'hgetall', 'hsetnx', 'hincrbyfloat', 'hscan', 'hstrlen', 'hrandfield'],
    'lists': ['lpush', 'rpush', 'lpop', 'rpop', 'llen', 'lrange', 'lindex', 'lset', 'ltrim', 'lrem', 'linsert', 'rpoplpush', 'brpoplpush', 'blpop', 'brpop', 'lmove', 'blmove', 'lpushx', 'rpushx'],
    'sets': ['sadd', 'srem', 'smembers', 'sismember', 'scard', 'sinter', 'sunion', 'sdiff', 'sinterstore', 'sunionstore', 'sdiffstore', 'spop', 'srandmember', 'smove', 'sscan', 'smismember', 'sintercard'],
    'sorted-sets': ['zadd', 'zrem', 'zrange', 'zrank', 'zscore', 'zcount', 'zincrby', 'zcard', 'zrevrange', 'zrevrank', 'zunion', 'zinter', 'zdiff', 'zrangebyscore', 'zremrangebyscore', 'zremrangebyrank', 'zscan', 'zpopmin', 'zpopmax', 'bzpopmin', 'bzpopmax', 'zrandmember', 'zunionstore', 'zinterstore', 'zintercard', 'zdiffstore'],
    ...
    'server': ['info', 'config', 'flushdb', 'flushall', 'dbsize', 'time', 'lastsave'],
    'bitmap': ['setbit', 'getbit', 'bitcount', 'bitpos', 'bitop', 'bitfield'],
    'hyperloglog': ['pfadd', 'pfcount', 'pfmerge'],
    'json': ['json'],
    'cluster': ['cluster', 'readonly', 'readwrite'],
    'generic': ['expire', 'expireat', 'ttl', 'pttl', 'persist', 'rename', 'renamenx', 'type', 'dump', 'restore', 'scan', 'keys', 'randomkey', 'touch', 'unlink', 'copy', 'object']
  };
...
function generateParams(methodName: string): string {
  ...
  if (name.startsWith('h')) {
    ...
    if (name === 'hgetall' || name === 'hkeys' || name === 'hvals') return '(key)';
...
function generateCompleteMappings() {
  ...
  allMethods.forEach(methodName => {
    ...
    mappingEntries.push({ ioredis: ioredisEntry, nodeRedis: nodeRedisEntry, methodName });
  });
  ...
  const comprehensiveDataset = {
    ...
    glide: {
      ...
      entries: mappingEntries.map(e => ({
        ...
        symbol: e.methodName,
        equivalent: { glide: `client.${e.methodName}${generateParams(e.methodName)}` },
  ...
  const outputPath = '/Users/avifen/valkey-glidejs-mcp/valkey-glidejs-mcp/src/data/api/comprehensive-mappings.ts';
...
`;
  ...
  console.log(`✅ Generated comprehensive mappings for ${allMethods.size} GLIDE APIs`);
